...
var rand = new SeededRandom(new Date().toISOString());//.substring(0,15));
...
function findHighest() {
  ...
  perms.map(function(perm, index) {
    if(index % 100000 == 0) log(index + '/' + perms.length);
...
fs.writeFileSync('./data.json', JSON.stringify({ score: score, highest: highestHands, deal: deal }, 0, '  '));
